...
in
{
  options.security.tpm2 = {
    ...
    fapi = {
      enable = lib.mkEnableOption "Enable using the TPM2 FAPI";
      ...
      systemDir = lib.mkOption {
        ...
        '';
        type = lib.types.str;
        default = "/usr/share/tpm2-tss/keystore";
      };
    ...
    lib.mkMerge [
...

      {
        systemd.services."tpm2-udev-trigger" = 
          let
            udevHash = if cfg.applyUdevRules then (builtins.hashString "md5" (udevRules cfg.tssUser cfg.tssGroup)) else "";
          in
          {
      ...
      (lib.mkIf cfg.fapi.enable (
        let
          fapiConfig = pkgs.writeText "fapi-config.json" (
            builtins.toJSON (
              {
              ...
              // lib.optionalAttrs (cfg.fapi.ekCertLess != null) { ek_cert_less = cfg.fapi.ekCertLess; }
